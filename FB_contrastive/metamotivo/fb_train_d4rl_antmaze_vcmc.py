...
from metamotivo.fb_vcmc import FBAgent, FBAgentConfig
...
def create_agent(
    ...
    agent_config.mcfb = (mode.lower() == "mcfb")
    agent_config.vcfb = (mode.lower() == "vcfb")
    ...
    def train_offline(self) -> None:
        ...
        for t in tqdm(range(0, int(self.cfg.num_train_steps))):
            ...
            if total_metrics is None:
                total_metrics = {
                    k: metrics[k].clone() if isinstance(metrics[k], torch.Tensor) else metrics[k]
                    for k in metrics.keys()
                }
            else:
                total_metrics = {
                    k: total_metrics[k] + (metrics[k].clone() if isinstance(metrics[k], torch.Tensor) else metrics[k])
                    for k in metrics.keys()
                }
            ...
            if t % self.cfg.log_every_updates == 0:
                ...
                for k in sorted(list(total_metrics.keys())):
                    ...
                    m_dict[k] = np.round(tmp.item() if isinstance(tmp, torch.Tensor) else tmp, 6)
