...
def train(cfg):
    ...
    work_dir = Path().cwd() / __LOGS__ / cfg.env / f'seed_{str(cfg.seed)}'
    ...
    if os.path.exists(data_dir):
        ...
        raise ValueError(f"Label not found")''' # This part is for uni-rlhf, we directly load the dataset below.
    
    with open(f"{cfg.data_dir}/{cfg.env}_pref_dataset.pkl", "rb") as f:
        pref_dataset = pickle.load(f)
    
    observation_dim = pref_dataset['observations'].shape[2]
    action_dim = pref_dataset['actions'].shape[2]
    ...
    if cfg.modality == "state":
        ...
        #print(pref_dataset['observations'].shape[2])
        #exit()
