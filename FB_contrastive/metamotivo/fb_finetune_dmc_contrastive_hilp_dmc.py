...
def load_preference_dataset(domain_task: str, dataset_path: str, dataset_type: str) -> dict:
    pref_dir = os.path.join(dataset_path, dataset_type)
    fname = f"{domain_task}_pref_dataset.pkl"
    #fname = f"{domain_task}_pref_dataset_K_100.pkl"
    fpath = os.path.join(pref_dir, fname)
    if not os.path.exists(fpath):
        raise FileNotFoundError(f"Preference dataset not found: {fpath}")
    with open(fpath, "rb") as f:
        batch = pickle.load(f)
    print(batch["next_observations"].shape, batch["next_observations_2"].shape, batch["labels"].shape)
    return batch
...
class Workspace:
    def __init__(self, cfg, agent_cfg):
        ...
        self.pref_dataset = load_preference_dataset(domain_task, self.cfg.dataset_path, pref_type)
