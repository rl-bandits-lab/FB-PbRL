...
class FBAgent:
    ...
    def sample_mixed_z(self, train_goal: torch.Tensor):
        ...
        if getattr(self.cfg.train, "use_z_noise", False):
            noise = torch.randn_like(z) * self.cfg.train.z_noise_std
            noise = torch.clamp(noise, -self.cfg.train.z_noise_clip, self.cfg.train.z_noise_clip)
            z = z + noise
