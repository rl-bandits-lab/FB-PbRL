...
def _soft_update_params2(net_params, target_net_params, tau):
    net_params = tuple(net_params)
    target_net_params = tuple(target_net_params)
    
    torch._foreach_mul_(target_net_params, 1 - tau)
    torch._foreach_add_(target_net_params, torch._foreach_mul(net_params, tau))
