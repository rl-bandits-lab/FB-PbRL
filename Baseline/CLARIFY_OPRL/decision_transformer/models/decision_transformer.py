...
class BidirectionalTransformer(TrajectoryModel):
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, target_states=None, target_masks=None, 
                target_timesteps=None, z_star=None):
        batch_size, seq_length = states.shape[0], states.shape[1]
        ...
        # use given z_star if available
        if z_star is not None:
            z = deepcopy(z_star)
            z = z.repeat(1, seq_length, 1)
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, target_states=None, target_masks=None, 
                   target_timesteps=None, z_star=None, **kwargs):
    ...
    def get_embedding(self, states, timesteps, attention_mask=None, target_states=None, target_masks=None, target_timesteps=None):
        z = self.get_full_embedding(states, timesteps, attention_mask, target_states, target_masks, target_timesteps)
        return z[:, 0, :]  # (batch_size, seq_length, z_dim) -> (batch_size, z_dim)

    def get_full_embedding(self, states, timesteps, attention_mask=None, target_states=None, target_masks=None, target_timesteps=None):
        batch_size, seq_length = states.shape[0], states.shape[1]
        ...
        if target_states is None:
            ...
            assert target_masks is not None
            assert target_timesteps is not None
            # target states are already reversed
            z = self.phi_transformer(states=target_states, timesteps=target_timesteps, attention_mask=target_masks)

        return z  # (batch_size, seq_length, z_dim)
...
class PhiTransformer(TrajectoryModel):
    ...
    def forward(self, states, timesteps, attention_mask=None):
        ...
        # reshape x so that the second dimension corresponds to
        # predicting states (1)
        x = x.reshape(batch_size, seq_length, 1, self.hidden_size).permute(0, 2, 1, 3)  # (batch_size, 1, seq_length, hidden_size)

        # get predictions
        z_preds = self.predict_z(x[:,0])  # (batch_size, seq_length, z_dim)
