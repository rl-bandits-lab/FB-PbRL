...
class TrainConfig:
    ...
    q_loss_coef: float = 0.0 # 0.1 for reward model
    batch_size: int = 1024
...
class FBAgent:
    ...
    @torch.no_grad()
    def sample_mixed_z(self, train_goal: torch.Tensor):
    ...
    @torch.no_grad()
    def preference_guide_z(self, obs: torch.Tensor, act: torch.Tensor, next_obs: torch.Tensor, reward_model):
        s_a = np.concatenate([
        ...
        B = self._model._backward_map(next_obs)

        z = torch.matmul(rewards.to('cuda').T, B)

        z = self._model.project_z(z)

        z = torch.repeat_interleave(z, self.cfg.train.batch_size, dim=0)  # batch x d
    ...
    #def update(self, replay_buffer, reward_model, step: int) -> Dict[str, torch.Tensor]:
    def update(self, replay_buffer, step: int) -> Dict[str, torch.Tensor]:
        ...
        '''if step < 1000000:
            ...
            z = self.preference_guide_z(obs, action, next_obs, reward_model).clone()'''
        self.z_buffer.add(z)
    ...
    def update_fb(
        ...
        if q_loss_coef is not None:
            with torch.no_grad():
                ...
                for member in range(3):
                    reward = reward_model.r_hat_member(s_a, member)
                    #reward = reward_model.ensemble[member](states.unsqueeze(1), actions.unsqueeze(1))
                    rewards.append(reward)
                rewards = torch.stack(rewards, dim=0).mean(dim=0)
                target_Q = reward.squeeze() + next_Qs'''
    ...
    def update_td3_actor(self, obs: torch.Tensor, z: torch.Tensor, clip_grad_norm: float) -> Dict[str, torch.Tensor]:
        ...
        if self.cfg.train.bc_coeff > 0:
            bc_error = F.mse_loss(action, action)
    ...
    @classmethod
    def load(cls, path: str, device: str):
        path = Path(path)
