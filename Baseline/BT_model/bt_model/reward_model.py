...
class TransformerRewardModel(RewardModel):
    ...
    def _train(self, batch, member):
        ...
        # to_torch
        obs_1 = utils_bt.to_torch(obs_1).to(self.device)
        act_1 = utils_bt.to_torch(act_1).to(self.device)
        obs_2 = utils_bt.to_torch(obs_2).to(self.device)
        act_2 = utils_bt.to_torch(act_2).to(self.device)
        ...
        y = utils_bt.to_torch(labels[:, :1], dtype=torch.float32).to(self.device) # batch_size * 1
